 ...
 * Module: short-hash function.
 * @(#) hash_sht.c	1.0	10mar2005 MJS
 *
...
/* functions to generate highly-distributed and highly-random unsigned-short
 ...
 * 893797-bit shorts (and so on), shorts of any size from byte to long
 * (whatever that is today), shorts of any size down to byte (whether that be
 * 8-bit, 9-bit or anything else), and so on and so on.
 *
 * As originally written, this code is good for 8-bit thru 1048576-bit shorts.
 * So don't mess with it unless you understand how that portability
 * has been achieved.
 *
 ...
 *
 * IMHO, the latter is guaranteed by the ANSI C Language standard,
 ...
 * and 37-bit shorts, *all* 37 bits are written, not just the bottom 32 bits.
 * This is very important, as hash functions are sometimes used to generate
 * random integers (by passing in random long strings of entropy) of which
 * only a certain number of bits are desired.
 * For instance, to generate a random number in the range 0-511, people
 * have been known to do something like:
 *
 *	char *s = get_entropy();
 *	short idx = rtw_short_hash(s, strlen(s)) & 0x1FF;
 *						 ^^^^^^^
 * which assumes that *all* bits returned by rtw_short_hash() are equally
...
/* compute an unsigned short hash value of specified (opaque) input key:
 */
USHORT rtw_short_hash(const BYTE *key, size_t len)
	{
	/* manifest constant number-of-octets-per-unsigned-short, *rounded up*
	 ...
	 */
	#define STRIDE ((sizeof(USHORT) / sizeof(BYTE)) + \
			((USHRT_MAX % OCTET_MAX) ? 1 : 0))

	USHORT output, octets[STRIDE];
	size_t n, i, j;
	...
	for (n = 0; n < STRIDE; ++n)
		...
		output |= octets[n];
		#ifdef DEBUG
		remark("rtw_short_hash output pass %i => 0x%0*hx",
